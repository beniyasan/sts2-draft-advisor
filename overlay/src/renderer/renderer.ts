...
const state: RendererState = { game: null, messages: [], busy: false, connected: false, model: null, modelOptions: [], error: null };
...
function render(): void {
  ...
  model.textContent = state.model ? `(${state.model})` : ""; status.textContent = state.error ? `エラー: ${state.error}` : state.connected ? (state.busy ? "相談中…" : "接続中") : "ゲーム接続待ち"; avatar.setState(state.error ? "error" : state.busy ? "thinking" : "idle");
}
window.draftAdvisor.onState((game: GameState) => { state.game = game; state.connected = true; render(); });
window.draftAdvisor.onDelta((delta: string) => { state.busy = true; const last = state.messages[state.messages.length - 1]; if (!last || last.role !== "assistant") state.messages.push({ role: "assistant", text: delta }); else last.text += delta; render(); });
window.draftAdvisor.onChatDone(() => { state.busy = false; render(); });
window.draftAdvisor.onStatus(update => { Object.assign(state, update); render(); });
document.getElementById("form")!.addEventListener("submit", async event => { event.preventDefault(); const question = input.value.trim(); if (!question || state.busy) return; input.value = ""; state.messages.push({ role: "user", text: question }); state.busy = true; state.error = null; render(); try { await window.draftAdvisor.ask(question); } catch (error) { state.busy = false; state.messages.push({ role: "system", text: String(error) }); render(); } });
